...
describe("CreateTransaction UseCase", () => {
  it("debería llamar a repo.create con la transacción proporcionada", async () => {
    const transactionMock = new Transaction("gateway-001", "credit_card", "visa", "Juan Pérez", "approved");

    const repoMock: TransactionRepository = {
      create: jest.fn().mockResolvedValue(undefined),
    };

    const useCase = new CreateTransaction(repoMock);
  ...
  it("debería lanzar un error si el repositorio falla", async () => {
    const transactionMock = new Transaction("gateway-002", "debit_card", "mastercard", "Ana López", "declined");
